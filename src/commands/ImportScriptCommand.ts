...
export class ImportScriptCommand implements Command {
    ...
    execute() {
        ...
        if (this.uri.fsPath.startsWith(modScriptsDir)) {
            if (fs.statSync(this.uri.fsPath).isDirectory()) {
                dirToListScripts = (this.uri.fsPath + "/").replace(modScriptsDir, path.dirname(baseScriptsPath));
            } else {
                dirToListScripts = path.dirname(this.uri.fsPath + "/").replace(modScriptsDir, path.dirname(baseScriptsPath));
            }
        }
